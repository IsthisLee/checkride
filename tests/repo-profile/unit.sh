...
printf '%s' "$out" | grep -q 'checked-practices'; check 0 $? "출력에 프로필 표시"
...
# 3. .checked-practices.toml 설정을 읽는다
...
out=$(NGG_LANG=ko run "$P3"); printf '%s' "$out" | grep -q 'checked-practices 프로필'; check 0 $? "ko: 한국어 머리글"
out=$(NGG_LANG=en run "$P3"); printf '%s' "$out" | grep -q 'checked-practices profile'; check 0 $? "en: 영어 머리글"
