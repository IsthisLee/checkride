...
rm -f "$Q/.checked-practices.toml"; AL="$T/allow"; mkdir -p "$AL/state/ti"
printf 'ti.skip\n' > "$AL/state/ti/allow"
...
grep -q 'checked-practices allow ti.skip' "$T/eal"; check 0 $? "허용: 막을 때 사람이 허용하는 법을 알린다"
printf 'ti.rm\n' > "$AL/state/ti/allow"
