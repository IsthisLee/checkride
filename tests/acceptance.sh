...
run setup-checks 'Read,Glob,Grep,Bash,Write,Edit' "/checkride:setup-checks"
...
run status 'Read,Glob,Grep,Bash' "/checkride:status"
grep -qE '게이트|gate' "$T/out.status"; check 0 $? "status: 게이트 상태를 보고한다"

run spec 'AskUserQuestion,Read,Glob,Grep,Write' "/checkride:spec 곱셈 함수를 더한다"
