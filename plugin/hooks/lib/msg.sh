...
msg_ko() { case "$1" in
  ...
  ngg.offbad)    M="  (checkride.toml 의 disabled_rules 에 없는 규칙 이름이 있다: %s. 아무것도 끄지 않았다.)" ;;
  ...
  done.nocmd)    M="코드 파일 %s개를 고쳤지만 검사 명령을 찾지 못했다. checkride.toml에 test_command를 적으면 이 턴부터 검사한다. 막지 않는다." ;;
  done.timeout)  M="검사가 제한 시간 %s초를 넘겨 시간초과로 중단했다(%s: %s). 판정하지 못했으므로 막지 않는다." ;;
  done.slow)     M="검사에 %s초 걸렸다. 턴마다 이만큼 기다리면 게이트를 꺼 버리게 된다. checkride.toml에 fast_test_command로 빠른 검사를 따로 적으면 턴 끝에는 그것만 돌리고 전체는 커밋 직전에 한 번 돌린다." ;;
  ...
  done.setup)    M="출력이 'command not found', 'Cannot find module', 'No such file' 같은 것이면 코드가 아니라 검사 설정 문제다. checkride.toml의 test_command나 프로젝트 설정을 보라." ;;
  done.pretime)  M="완료 게이트: 커밋 전 전체 검사가 %s초를 넘겨 중단했다. 판정하지 못했으므로 막지 않는다." ;;
  done.prehead)  M="완료 게이트: 커밋 전 전체 검사가 실패했다(exit %s). 커밋할 수 없다." ;;
  done.preran)   M="- 돌린 명령: %s   (checkride.toml test_command)" ;;
  ...
  pg.conf)       M="- 설정: append_only = \"%s\"  (checkride.toml)" ;;
  pg.appendtail) M="- 고쳐야 할 내용이 있으면 지난 파일을 바꾸지 말고 새 파일을 더해라." ;;

  line.cmd)      M="- 명령: %s" ;;
  line.file)     M="- 파일: %s" ;;
  line.target)   M="- 대상: %s" ;;
  line.allow)    M="- 사람이 이번 한 번만 넘기려면 다음 프롬프트에 check allow %s 를 한 줄로 쓴다. 한 번 쓰면 사라진다." ;;

  rp.head)       M="[check 프로필] %s" ;;
  rp.branch)     M="  (브랜치 %s)" ;;
  rp.pm)         M="패키지 매니저: %s" ;;
  rp.stack)      M="스택: %s" ;;
  rp.cmd)        M="검사 명령: %s   (출처: %s)" ;;
  rp.nocmd)      M="검사 명령을 찾지 못했다. checkride.toml에 test_command를 적으면 완료 게이트가 이 저장소에서 동작한다." ;;
  ...
  rp.disabled)   M="끈 규칙: %s  (checkride.toml 의 disabled_rules)" ;;
...
msg_en() { case "$1" in
  ...
  ngg.offbad)    M="  (disabled_rules in checkride.toml names an unknown rule: %s. Nothing was disabled.)" ;;
  ...
  done.nocmd)    M="%s code file(s) changed, but no check command was found. Add test_command to checkride.toml and the check runs from the next turn. Not blocking." ;;
  done.timeout)  M="The check ran past its %ss limit and was stopped (%s: %s). There is no verdict, so this is not blocking." ;;
  done.slow)     M="The check took %ss. Waiting that long every turn is how a gate ends up switched off. Put a quick check in checkride.toml as fast_test_command: the end of a turn then runs only that, and the full suite runs once, right before a commit." ;;
  ...
  done.setup)    M="If the output says 'command not found', 'Cannot find module' or 'No such file', this is the check setup, not your code. Look at test_command in checkride.toml, or at the project setup." ;;
  ...
  done.preran)   M="- Command: %s   (checkride.toml test_command)" ;;
  ...
  pg.conf)       M="- Setting: append_only = \"%s\"  (checkride.toml)" ;;
  ...
  line.allow)    M="- To let this through once, a human writes check allow %s on its own line in the next prompt. It is used up after one pass." ;;

  rp.head)       M="[check profile] %s" ;;
  rp.branch)     M="  (branch %s)" ;;
  rp.pm)         M="Package manager: %s" ;;
  rp.stack)      M="Stack: %s" ;;
  rp.cmd)        M="Check command: %s   (from: %s)" ;;
  rp.nocmd)      M="No check command found. Add test_command to checkride.toml to turn the completion gate on for this repo." ;;
  ...
  rp.disabled)   M="Disabled rules: %s  (disabled_rules in checkride.toml)" ;;
