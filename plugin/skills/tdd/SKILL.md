---
name: tdd
description: 실패하는 테스트를 먼저 쓰고, 실패를 눈으로 확인한 뒤 구현한다. 테스트 무결성 게이트가 무력화 시도를 막는다.
...
통과하지 않는 테스트는 **테스트를 고칠 이유가 되지 않는다.** 테스트 무결성 게이트가 `.skip` 추가와 단언 삭제를 편집 시점에 막는다. 게이트가 막으면 게이트가 옳다고 전제한다.
