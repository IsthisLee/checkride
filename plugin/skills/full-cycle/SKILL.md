...
**plan mode** (`Shift+Tab`) 로 들어가 계획을 쓴다. 승인 전에는 코드를 바꾸지 않는다. 계획해 보니 큰 기능이라면 `/checkride:spec` 으로 스펙부터 확정하자고 제안한다.

## 3. 구현

`/checkride:tdd` 의 순환을 따른다. 실패 테스트 먼저, RED 확인, 최소 구현, GREEN 확인이다.
...
검사와 커밋과 PR 은 `/checkride:finish` 로 한다. 세션이 여기서 끝난다면 `/checkride:handoff` 도 돌린다.
