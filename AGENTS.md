...
- **메시지 언어는 로케일을 따른다.** `NGG_LANG`이 우선하고, 없으면 `checkride.toml`의 `lang`, 그것도 없으면 `LC_ALL` → `LC_MESSAGES` → `LANG` 순으로 본다. `ko` 계열이면 한국어, 그 외에는 영어다. 단위 테스트는 머리에서 `NGG_LANG=ko`를 못 박아 기계마다 결과가 달라지지 않게 한다.
