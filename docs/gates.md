...
**커맨드는 사용자가 쓴 언어로 답합니다.** `SKILL.md`는 로케일로 가를 수 없는 파일이라 영어로 쓰고, 본문에 사용자의 언어로 답하라는 지시를 넣어 두었습니다. 게이트 문장이 `msg.sh`에서 언어별로 갈리는 것과 같은 원칙입니다.
