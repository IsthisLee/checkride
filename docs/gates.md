...
게이트 넷과 저장소 프로필은 서로 다른 훅 이벤트에 걸린다. **Stop(턴 끝)에서 도는 것은 근거 게이트와 완료 게이트 둘뿐이다.** 테스트 무결성과 프로젝트 가드는 턴이 끝날 때가 아니라 편집·Bash를 실행하려는 순간에만 막는다. 배선은 `plugin/hooks/hooks.json`에 있다.
...
근거 게이트의 Stop 규칙은 조건이 맞는 것만 본다. 대부분 **이 턴에 도구를 안 썼을 때**만 걸린다.
...
| R4 | 직전 턴에 막혔고 그 뒤로 도구를 안 썼을 때만 |
| R2b | 프롬프트가 로컬 상태를 묻는 맥락일 때만 |

그래서 도구를 쓰고 근거를 붙여 답한 흔한 턴에서는 걸릴 것이 없어 그냥 통과한다.

### 규칙을 보기 전에 빠지는 면제

Stop 훅은 규칙을 따지기 전에 두 가지를 먼저 본다. 하나라도 맞으면 규칙을 아예 안 본다.

- 답이 질문형이거나 이 턴에 `AskUserQuestion`을 썼다. 묻는 것은 언제나 허용한다.
- 판정기가 띄운 중첩 세션이다. 게이트가 자기 안에서 또 돌지 않게 막는다.

규칙을 본 뒤 풀어 주는 면제(불가·JSON·인용·의견)는 [README](../README.md#걸리지-않는-것)에 있다.
