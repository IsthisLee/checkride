...
## V62 README 에 커맨드 절을 신설하고 수량 표현에 분류사를 넣는다

README 가 26,363자였는데 **커맨드 여덟 개를 소개하는 절이 없었다.**

```
$ grep -o "스킬" README.md | wc -l        → 0
/check:spec  /check:handoff  /check:status  /check:auto  → 각 0회
R0~R5 언급 45회  vs  다른 게이트 이름 언급 4회
```

선언은 "모범 사례를 지키게 한다" 인데 분량이 그것을 배신했다. 첫 문장도 "모범 사례를 지켰는지 턴마다
확인합니다" 로 게이트만 말했다. **턴마다 도는 것은 게이트이고 커맨드는 사용자가 직접 부른다.**

### 머리말을 세 번 고쳤다

처음 안은 "검증된 모범 사례를 절차로 주고, 지키지 않으면 막습니다" 였다. 메커니즘만 말하고 결과를
말하지 않는다는 지적을 받아 바꿨다.

```
Claude Code 작업의 신뢰성을 높입니다.

커맨드 여덟 개가 검증된 모범 사례를 절차로 주고, 게이트 네 개가 그 절차를 지키지 않은 답을 막습니다.
```

「신뢰성」은 린터·CI·리뷰 도구도 하는 말이라 그 자체로는 차별점이 없다. **그래서 바로 아래에 두 기둥
표를 두고 사이에 아무것도 끼우지 않았다.** 표에서 갈리는 것은 「직접 쳐야 돕니다」 대 「저절로 돕니다」다.
사람이 부르는 절차와 저절로 걸리는 제지가 한 도구에 있다는 것이 다른 도구와 다른 점이다.

### R0~R5 표를 내리려던 계획이 틀렸다

`docs/gates.md:99` 가 이렇게 적는다.

```
규칙과 면제는 [README](../README.md#무엇이-막히나)에 있습니다.
```

**상세 문서가 README 를 정본으로 가리킨다.** 내리면 링크가 순환한다. 역할이 이미 나뉘어 있었다.
README 가 「무엇을 막는가」, `gates.md` 가 「어떻게 도는가」다. 대신 「근거」 절(4,833자)이 진짜 중복이라
내렸다. 검토자가 출처 7개가 `gates.md` 쪽 9개의 부분집합임을 대조해 유실이 없음을 확인했다.

### 자기 정직성 선언은 남겼다

「알려진 미탐」 표는 `docs/decisions.md` 4절로 내리되 첫 문장은 README 에 두었다.

```
막지 못하는 것도 함께 적어 둡니다.
오탐만 적고 미탐을 숨긴다면, 그것 자체가 근거 없는 주장이 되기 때문입니다.
```

**근거 없는 주장을 막으려는 도구가 자기 한계를 감추면 자기모순이다.**

### 수량 표현에 분류사를 넣었다

한국어에서 사물을 셀 때는 분류사가 필요하다. 「게이트 넷이」가 아니라 「게이트 네 개가」다. 사람을 셀
때만 수사가 명사에 바로 붙는다(「사람 셋이」).

**처음에는 제목을 예외로 두려 했다.** `## 커맨드 여덟` 을 고치면 앵커가 바뀌고 참조하는 곳도 따라
고쳐야 하기 때문이다. 그것이 문법 판단이 아니라 수고를 피하려는 변명이라는 지적을 받고 제목도 고쳤다.
전역 지침에도 그 변명을 막는 문장을 넣었다. "그 수고를 피하려고 어색한 제목을 남기지 않는다."

인용은 고치지 않았다. `CHANGELOG.md` 의 `[Unreleased]` 절이 **무엇이 틀렸는지 보여 주려고** 옛 표현을
따옴표로 인용하고 있어서다. 고치면 문장이 뜻을 잃는다. 인용과 서술을 갈라 세는 검사를 만들어 확인했다.
...
```
README.md      26,363 → 19,634자
README.en.md   25,397 → 23,036자
앵커           README.md 12개 · README.en.md 12개 · docs/gates.md 19개, 깨진 것 0개
전체 검사      실패 줄 0, 종료 코드 0
```

### 판단

- **측정 도구가 네 번 틀렸다.** `sed` 의 `\|` 가 BSD 에서 리터럴로 처리된 것, `--plugin-dir` 상대 경로로
  플러그인이 로드되지 않은 것, `git diff` 가 스테이지된 변경을 보이지 않은 것, 정규식이 「훅 열두 개」의
  `열` 을 잡은 것이다. **넷 다 대상이 아니라 도구가 틀렸는데 대상이 틀렸다고 읽을 뻔했다.**
...
## V64 문서가 적은 검사 건수를 검사로 고정하고, 머리말의 대상을 바로잡는다

두 가지를 함께 고쳤다. 둘 다 이 저장소가 스스로 어긴 규칙이다.

### 숫자를 지키는 검사가 자기 숫자는 세지 않았다

`tests/invariants.sh` 는 문서의 숫자를 여럿 검사한다. 커맨드가 여덟인지, 훅 모듈이 다섯인지,
`plugin/` 파일이 25개인지, 퍼징이 240회인지다. **그런데 자기 검사 건수를 세는 장치가 없었다.**

V63 에서 `invariants.sh` 에 검사를 하나 더하고 `AGENTS.md` 의 `20건`·`448건` 을 고치지 않았다.
검토자가 직접 세어 보지 않았다면 틀린 채로 병합됐다.

```
$ git show b009dc9:tests/invariants.sh > /tmp/inv.sh; bash /tmp/inv.sh | grep -c '^✅\|^❌'
20
$ bash tests/invariants.sh | grep -c '^✅\|^❌'
21
```

### 두 번 실패하고 세 번째로 갔다

**1차: `invariants.sh` 안에 넣기.** 그 검사는 단위 테스트를 실제로 돌려 건수를 세는데, 목록에
`invariants.sh` 자신이 있어 **무한 재귀**가 됐다. 출력이 비어서 돌아왔다.

**2차: `check` 호출 수를 정적으로 세기.** 실행 없이 세려 했으나 실제와 전혀 맞지 않았다.

```
no-guess-gate/unit.sh   정적 7건   실제 170건
done-gate/unit.sh       정적 1건   실제  83건
test-integrity/unit.sh  정적 0건   실제  69건
```

`check` 가 루프와 함수 안에서 불리기 때문이다.

**3차: `tests/doc-counts.sh` 로 분리.** 전체 검사 명령의 맨 끝에 한 번만 둔다. 재귀가 없고, 앞에서
이미 돈 테스트를 다시 돌리는 비용만 남는다. **문서의 숫자가 조용히 낡는 것이 그보다 비싸다.**

**같은 재귀를 두 번 만들었다.** 3차에서도 목록에 자기 자신을 넣었다가 되돌렸다. 「이 검사가 모든
테스트를 실행한다」와 「이 검사도 테스트다」를 동시에 들고 있지 못했다. 지금은 목록에서 빼고 합계에
상수 1을 더하며, 왜 그런지를 코드 주석에 박아 두었다.

### 검사가 실제 오염에서 두 번 잡았다

구분력을 따로 재려던 참에 진짜 오염이 생겼다. 파일을 고쳤다 되돌리는 검증을 **백그라운드로 돌렸고,
복구되기 전에 같은 파일을 편집했다.** `cp $B AGENTS.md` 가 파일 전체를 덮어 제 편집을 지웠다.

```
❌ tests/no-guess-gate/unit.sh: 문서 171건, 실제 170건
❌ 합계: 문서 449건, 실제 450건
```

**만들려던 검사가 바로 그 사고를 잡았다.** 의도한 구분력 테스트보다 나은 증거다.

`git diff` 로 매번 오염을 정확히 좁힐 수 있었다. 커밋하지 않은 변경이라 `HEAD` 가 기준선 역할을 했다.
**중간에 커밋했다면 오염된 상태가 기준선이 되어 찾기 어려웠다.**

### 머리말이 게이트의 대상을 잘못 적었다

```
전   커맨드 여덟 개가 … 절차로 주고, 게이트 네 개가 그 절차를 지키지 않은 답을 막습니다
```

**게이트는 커맨드를 쓰든 안 쓰든 돈다.** 훅 배선을 실측하니 커맨드나 스킬을 조건으로 삼는 곳이 0건이고,
걸리는 이벤트는 전부 세션과 도구 호출에 붙는 것이다.

```
$ grep -c 'skills\|커맨드' plugin/hooks/hooks.json
0
$ python3 -c "import json; print(' '.join(json.load(open('plugin/hooks/hooks.json'))['hooks'].keys()))"
UserPromptSubmit PreToolUse Stop SubagentStop PostToolUse SessionStart PostToolUseFailure
```

「그 절차」라고 쓰면 **커맨드를 쓴 경우에만 막는 것처럼** 읽힌다. 커맨드는 모범 사례를 절차로 풀어 주는
것이고 게이트는 모범 사례를 지켰는지 검사하는 것이다. **둘의 공통 대상은 모범 사례이지 절차가 아니다.**

```
후   검증된 모범 사례를 커맨드 여덟 개가 절차로 풀어 주고,
     게이트 네 개가 지켜졌는지 턴마다 검사해 지키지 않은 답으로는 턴이 끝나지 않게 합니다
```

「검사합니다」로만 줄이지 않았다. **막는다는 성격이 린터·CI 와 갈리는 지점**이라 둘을 다 살렸다.
표의 게이트 행과 영어판 두 곳도 같은 기준으로 고쳤다.
...
$ shellcheck -x -s bash tests/doc-counts.sh
(종료 코드 0)

$ grep -n '그 절차\|that procedure\|skipped it' README.md README.en.md
(0건)
```

### 판단

- **같은 재귀를 두 번 만들었다.** 검사가 자기 자신을 실행 대상으로 삼는 구조는 한 번 겪고도 다시
  놓쳤다. 그래서 코드 주석에 「이 파일 자신도 목록에 넣지 않는다」를 명시했다.
- **파일을 고쳤다 되돌리는 검증은 백그라운드로 돌리면 안 된다.** 되돌리기 전에 다른 편집이 끼어들면
